...
func main() {
	...
	producer, err := kafka.NewProducer(&kafka.ConfigMap{
		...
		"ssl.key.location":         "security/SASL_SSL/certs/producer.key",        // Path to client's private key (PEM) used for authentication.
		"ssl.certificate.location": "security/SASL_SSL/certs/producer-signed.crt", // Path to client's public key (PEM) used for authentication.
		"ssl.ca.location":          "security/SASL_SSL/certs/rootCA.crt",          // Path to CA certificate.
	...
	default:
		slog.Error("Ignored event", slog.String("event", ev.String()))
	}
